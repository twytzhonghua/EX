...
static void buttons_timer_function(unsigned long data)
{
	...
	if(!ppin_desc)
		return ;
	//val_temp = gpio_get_value(ppin_desc->pin);
	val_temp = s3c2410_gpio_getpin(ppin_desc->pin);
	if(0 == val_temp)
	{

//		input_report_key(button_dev, ppin_desc->val, 0);
...
{ 
	...
	for(i=0;i<3;i++)
	{
		if (request_irq(pin_desc[i].irq, button_interrupt, IRQT_BOTHEDGE, pin_desc[i].name, &pin_desc[i]))
		{ 
